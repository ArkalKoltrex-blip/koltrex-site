...
description: >
  ...
  pedir explicitamente "rodar /instalar", "instalar o ArkhTrex Framework", "primeiro setup".
---

# /instalar — Instalação inicial do ArkhTrex Framework
...
Conferir o nome da pasta atual (`basename "$(pwd)"`). Se for `kwt-framework`, `kwt framework` ou variação genérica:

> "Notei que a pasta atual ainda tem nome genérico ('<nome-atual>'). O ideal é a pasta ter o nome do seu negócio. Quando terminarmos o setup, te lembro de renomear (é rápido — fechar VS Code, renomear a pasta no Finder/Explorer, abrir de novo). Bora seguir?"
...
A resposta determina como o `CLAUDE.md` vai ser adaptado ao perfil escolhido.
...
### `CLAUDE.md`
Adaptar o `CLAUDE.md` da raiz com o nome do negócio, perfil escolhido e estrutura de pastas mencionada nas respostas. Sobrescrever apenas a seção de contexto do negócio, mantendo as regras de comportamento intactas.
...
> "Pronto. O ArkhTrex Framework já te conhece.
>
